...
public class GamePrinter {
	...
	private static final String SUPERCOIN_MSG = "SuperCoin is present ";
	...
	protected String getInfo() {
		...
		if(game.getThunder()) {
			buffer.append("Thunder hit position: (" +  (game.getThunderX()) + ", " + (game.getThunderY()) + ")");
			if(game.isInPosition(game.getThunderY(), game.getThunderX())){
				buffer.append(" -> " + game.getDeleteThunder() + "\n");
			}
		}
		buffer
		.append(DISTANCE_MSG).append(game.distanceToGoal() + "\n")
		.append(COINS_MSG).append(game.playerCoins() + "\n")
		.append(CYCLE_MSG).append(game.getCycle() + "\n")
		.append(TOTAL_OBSTACLES_MSG).append(Obstacle.getObstaclesCount() + Wall.getObstaclesCount() 
		+ Pedrestrian.getObstaclesCount() + Truck.getObstaclesCount() + "\n")
		...
		if (this.superCoin) {
			buffer.append(SUPERCOIN_MSG + "\n");
			this.superCoin = false;
		}
		if (!game.isTestMode()) {
			/* @formatter:off */
			buffer.append(ELAPSED_TIME_MSG).append(formatTime(game.elapsedTime()));
			/* @formatter:on */
		}
	...
	public String toString() {
		...
		for (int y = 0; y < game.getWidth(); y++) {
			...
			for (int x = game.getColumnaPlayer(); x < game.getColumnaPlayer() + game.getVisibility(); x++) {
				if(game.positionToString(x, y) == "$") { this.superCoin = true;}
	...
	public String endMessage() {
		...
		if (game.hasCrashed()) {
			return buffer.append(CRASH_MSG).toString();
		}
		...
		if (game.hasArrived()) {
			...
			if (!game.isTestMode()) {
				if (game.isNewRecord()) {
					...
					buffer.append(RECORD_MSG).append(formatTime(game.elapsedTime()));
				}
			}
		} else {
			buffer.append(DO_EXIT_MSG);
		}
